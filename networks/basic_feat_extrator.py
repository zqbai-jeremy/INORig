...
class RGBNet(nn.Module):
    def __init__(self, input_dim=(3, 256, 256)):
        ...
        drn_module = drn.drn_d_38(pretrained=False)  # use DRN38 for now
...
class RGBNetSmall(nn.Module):
    def __init__(self, input_dim=(3, 256, 256)):
        """
        Small network for extracting RGB features
        :param input_dim: size of the input image
        """
        super(RGBNetSmall, self).__init__()
        self.input_shape_chw = input_dim

        drn_module = drn.drn_d_22(pretrained=False)  # use DRN22 for now
        self.block0 = nn.Sequential(
            drn_module.layer0,
            drn_module.layer1
        )
...
class RGBNetSmallSmall(nn.Module):
    def __init__(self, input_dim=(3, 256, 256)):
        """
        Small network for extracting RGB features
        :param input_dim: size of the input image
        """
        super(RGBNetSmallSmall, self).__init__()
        self.input_shape_chw = input_dim
